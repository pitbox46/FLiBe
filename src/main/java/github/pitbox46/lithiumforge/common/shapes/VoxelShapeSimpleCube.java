package github.pitbox46.lithiumforge.common.shapes;
...
public class VoxelShapeSimpleCube extends VoxelShape implements VoxelShapeCaster {
    ...
    private double calculatePenetration(AxisCycle dir, AABB box, double maxDist) {
        return switch (dir) {
            case NONE -> VoxelShapeSimpleCube.calculatePenetration(this.minX, this.maxX, box.minX, box.maxX, maxDist);
            case FORWARD ->
                    VoxelShapeSimpleCube.calculatePenetration(this.minZ, this.maxZ, box.minZ, box.maxZ, maxDist);
            case BACKWARD ->
                    VoxelShapeSimpleCube.calculatePenetration(this.minY, this.maxY, box.minY, box.maxY, maxDist);
    ...
    boolean intersects(AxisCycle dir, AABB box) {
        return switch (dir) {
            case NONE ->
                    lessThan(this.minY, box.maxY) && lessThan(box.minY, this.maxY) && lessThan(this.minZ, box.maxZ) && lessThan(box.minZ, this.maxZ);
            case FORWARD ->
                    lessThan(this.minX, box.maxX) && lessThan(box.minX, this.maxX) && lessThan(this.minY, box.maxY) && lessThan(box.minY, this.maxY);
            case BACKWARD ->
                    lessThan(this.minZ, box.maxZ) && lessThan(box.minZ, this.maxZ) && lessThan(this.minX, box.maxX) && lessThan(box.minX, this.maxX);
            default -> throw new IllegalArgumentException();
        };
    }
